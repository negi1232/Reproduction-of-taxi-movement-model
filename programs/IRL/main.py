...
class maxent():
    ...
    def irl(self,trajectories,states,n):
        ...
        #状態ごとの単位ベクトル
        state_features = np.vstack([self.state_to_feature(s)
        ...
        self.value_iteration = ValueIteration(states, 5)#4,list
        self.n_actions=4
        self.n_states=states
        learning_rate=0.1
        ...
        for e in tqdm(range(n)):
            ...
            estimated = estimated.reshape((int(math.sqrt(states)),int(math.sqrt(states))))
            plt.close()
    ...
    def list2excel(self,array,array2,name):
        ...
        for i in range(len(array)):
            for j in range(len(array)):
                ...
                sheet.cell(row=(i*len(array))+j+1, column=3, value=float(array2[i*self.n_states+j]))
    ...
    def policy2feature(self,policy,trajectories):
        ...
        for t in range(1, t_size):
            ...
            for prev_s in range(states):#各状態から次のステップをまわす
                ...
                # 状態遷移確率に従い、各状態への遷移確率を取得. probsは16次元のリスト.最大エントロピー法を用いているはず
...
if __name__ == "__main__":
    ...
    trajectories,states=db2expart.db2trajectory(hour,60)
    os.chdir(os.path.dirname(os.path.abspath(__file__)))
    ...
    Irl.irl(trajectories,states, 50)
