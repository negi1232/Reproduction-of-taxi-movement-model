...
class Vis():
    ...
    def __init__(self):
        ...
        self.startmark.thumbnail((9, 9))
        self.goalmark.thumbnail((9, 9))
    ...
    def synthetic(self,route,time):
        ...
        # for i in range(60):
        #     img.paste(self.goalmark, (self.x_list[goal[1]] ,self.y_list[ac-1-i]), self.goalmark)
...
def import_reward():#エクセルからデータを抽出する
    ...
    for h in range(24):
        wb = openpyxl.load_workbook("./input_data/result/result"+str(h)+".xlsx")
        ws = wb.worksheets[0]
        x=0
        y=0
...
def import_data(name):#SQLからデータを抽出する
    ...
    cur = con.cursor()
    cur.execute('SELECT * FROM '+str(name) +' ORDER BY id DESC LIMIT 3000')
