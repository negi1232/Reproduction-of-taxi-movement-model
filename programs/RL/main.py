...
if __name__ == '__main__':
    ...
    for i in range(10):
        agents.append(Q_agent(envs,qtable,states,start_pos,(i)/10))#,env,actions,q_table,state
    ...
    max_reward=0
    for e in envs:
        if max_reward<= int(max(e.pos_reward)):
            max_reward= int(max(e.pos_reward))
    for i in tqdm( range(epoch)):
        ...
        for j,agent in enumerate( agents):
            ...
            while envs[int(agent.hour)].pos_reward[previous_pos]/ int(max(envs[int(agent.hour)].pos_reward))<=random.random():
                #print(envs[int(agent.hour)].pos_reward[previous_pos]/int(max(envs[int(agent.hour)].pos_reward)),random.random())
                route_st.insert(0, s_p)
                step+=1
